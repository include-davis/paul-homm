export default async function fetchHomepageData(req, res){
    if(req.method === 'POST' && req.body.locale ){
        ...
        try{
            ...
            const homepageRes = await (await fetch(`${process.env.CMS_BASE_URL}/api/homepage?locale=${locale}&populate=*`, {
            method: 'GET',
            headers: {
                Authorization:
            ...
            const closureDatesRes = await (await fetch(`${process.env.CMS_BASE_URL}/api/closure-dates?populate=*&publicationState=live&filters[$and][0][closure_date][$gte]=${today}`, {
                method: 'GET',
                headers: {
                    Authorization:
                    `Bearer ${process.env.CMS_API_KEY}`,
            ...
            if(closureDatesRes.data){
                ...
                closureDates = dates.map((dateDoc)=>{
                    ...
                    const now = new Date();
                    const date = now.getTimezoneOffset() > 420 ?  `${dateString}T00:00:00-08:00` : `${dateString}T00:00:00-07:00`;
                    return date;
                });
            } else {
            ...
            // upcoming events
            const eventsRes = await (await fetch(`${process.env.CMS_BASE_URL}/api/upcoming-events?locale=${locale}&populate=*&publicationState=live&filters[$and][0][event_date][$gte]=${today}`, 
            {
                method: 'GET',
                headers: {
                    Authorization:
            ...
            if(eventsRes.data){
                ...
                upcomingEvents = events.map((eventDoc)=>{
                    ...
                    const now = new Date();
                    const dateString = now.getTimezoneOffset() > 420 ?  `${event_date}T00:00:00-08:00` : `${event_date}T00:00:00-07:00`;
                    return {
                        date: dateString,
                        event: event_title,
                    }
